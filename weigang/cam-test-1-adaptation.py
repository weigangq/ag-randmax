...
parser = argparse.ArgumentParser(
    description="Simulate coevolutionary automata model (CAM) of John Miller. Test 1: evolving machines adapted to a predefined random string")
...
parser.add_argument('-asize', '--automata_size', type=int, default=5,
                    help='Number of states in a automata. Default = 5.')
                    
parser.add_argument('-arch', '--archive_method', choices=['1', '2'], default='2',
...
parser.add_argument('-pick', '--pickle_last', action = 'store_true',
                    help='pickle the last population for visualization and validataion')
...
for n in range(args.generation):
    ...
    # print(f"{args.tag}\t{args.target_size}\t{round(p.elite1[2],4)}\t{p.target}\t{p.elite1[1]['output']}\t{p.avg_fit}\t{args.automata_size}\t{args.algorithm}")
    p.mutate()
    if args.algorithm == '1':
        p.elite_selection()
    if args.algorithm == '2':
...
if args.pickle_last:
    with open("last_pop.pickle", 'wb') as f:
        pickle.dump(p, f)
